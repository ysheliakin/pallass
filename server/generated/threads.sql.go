...
type GetThreadAndMessagesByThreadIDAndFullnameByUserEmailRow struct {
	...
	FundingOpportunityTitle pgtype.Text
	UpvoteCount             int64
...
const getThreadCategoriesAndFundingOpportunities = `-- name: GetThreadCategoriesAndFundingOpportunities :many
SELECT DISTINCT t.category, fo.id, fo.title
FROM threads t
CROSS JOIN funding_opportunities fo
`

type GetThreadCategoriesAndFundingOpportunitiesRow struct {
	Category string
	ID       int32
	Title    string
}

func (q *Queries) GetThreadCategoriesAndFundingOpportunities(ctx context.Context) ([]GetThreadCategoriesAndFundingOpportunitiesRow, error) {
	rows, err := q.db.Query(ctx, getThreadCategoriesAndFundingOpportunities)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetThreadCategoriesAndFundingOpportunitiesRow
	for rows.Next() {
		var i GetThreadCategoriesAndFundingOpportunitiesRow
		if err := rows.Scan(&i.Category, &i.ID, &i.Title); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
...
SELECT id, title, content, category, created_at, uuid, user_email, funding_opportunity_id 
FROM threads
WHERE category = $1
...
SELECT
    threads.id, threads.title, threads.content, threads.category, threads.created_at, threads.uuid, threads.user_email, threads.funding_opportunity_id,
    COUNT(thread_upvotes.id) AS upvote_count
FROM 
    threads
LEFT JOIN
...
SELECT threads.id, title, content, category, threads.created_at, uuid, threads.user_email, funding_opportunity_id, thread_upvotes.id, thread_id, thread_upvotes.created_at, thread_upvotes.user_email
FROM threads
...
const insertThreadWithGrant = `-- name: InsertThreadWithGrant :one
INSERT INTO threads (title, content, category, user_email, created_at, funding_opportunity_id)
VALUES ($1, $2, $3, $4, CURRENT_TIMESTAMP, $5)
RETURNING id, uuid
`

type InsertThreadWithGrantParams struct {
	Title                string
	Content              string
	Category             string
	UserEmail            pgtype.Text
...
type InsertThreadWithGrantRow struct {
	ID   int32
	Uuid pgtype.UUID
}

func (q *Queries) InsertThreadWithGrant(ctx context.Context, arg InsertThreadWithGrantParams) (InsertThreadWithGrantRow, error) {
	row := q.db.QueryRow(ctx, insertThreadWithGrant,
		arg.Title,
		arg.Content,
		arg.Category,
		arg.UserEmail,
		arg.FundingOpportunityID,
	)
	var i InsertThreadWithGrantRow
	err := row.Scan(&i.ID, &i.Uuid)
	return i, err
}
