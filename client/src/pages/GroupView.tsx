...
export function GroupView() {
  ...
  const fetchGroup = async () => {
    ...
    ws.current = new WebSocket(`${wsBase}/wsgroup/${email}`);
    ...
    ws.current.onmessage = (event) => {
      ...
      if (message.type === 'EDIT_MESSAGE') {
        ...
        setGroupData((prevMessages) => {
          ...
          const updatedGroupMessages = prevMessages.map((msg) =>
            msg.GroupMessageID == message.id
              ? { ...msg, GroupMessageContent: message.content }
              : msg
          );
      ...
      <Container size="lg" mt={30}>
                  ...
                  <Group style={{ justifyContent: 'center' }}>
                    ...
                    <Group>
                      <Button
                        variant="subtle"
                        color="green"
                        onClick={() => acceptJoinRequest(request.UserEmail)}
                      >
                        Accept
                      </Button>
                      <Button
                        variant="subtle"
                        color="red"
                        onClick={() => denyJoinRequest(request.UserEmail)}
                      >
                        Deny
                      </Button>
                    </Group>
                  ...
                  <Group>
                    ...
                    {email == member.UserEmail && member.Role != 'Owner' ? (
                      <Button
                        ...
                        variant="subtle"
                        color="red"
                        onClick={() => leaveGroup(member.UserEmail)}
                      >
                        Leave
                      </Button>
                    ) : null}
                    ...
                    {email == groupOwner && email != member.UserEmail ? (
                      <Group>
                        <Button
                          variant="subtle"
                          color="red"
                          onClick={() => setNewOwner(email, member.UserEmail)}
                        >
                          Set as Owner
                        </Button>
                        <Button
                          variant="subtle"
                          color="red"
                          onClick={() => kickMemberOut(member.UserEmail)}
                        >
                          Kick Out
                        </Button>
                      </Group>
          ...
          <Stack align="center">
            ...
            <Group>
              <TextInput
                ...
                style={{ width: '400px' }}
              />
              <Button color="red" onClick={() => addMember(newMember)}>
                Add Member
              </Button>
            </Group>
          </Stack>
        ...
        >
          ...
          <Stack align="center">
            ...
            <Button color="red" onClick={confirmGroupDeletion}>
        ...
        >
          ...
          <Group justify="space-between" align="center">
            <Text size="sm" color="dimmed">
              Created on:{' '}
              <strong>{new Date(groupData[0].GroupCreatedAt).toLocaleDateString()}</strong>
            </Text>
          </Group>
          ...
          {email == groupOwner && (
            <div style={{ textAlign: 'right' }}>
              ...
              <Button
                ...
                variant="subtle"
                color="red"
                onClick={openDeleteGroupVerification}
              >
                Delete Group
              </Button>
            </div>
                ...
                <React.Fragment key={groupMessage.GroupMessageID}>
                        ...
                        >
                          <Text>
                            ...
                            <span style={{ fontSize: 12, fontWeight: 700 }}>
                              {groupMessage.GroupReplyFirstname} {groupMessage.GroupReplyLastname}
                            </span>{' '}
                            <span style={{ fontSize: 10, float: 'right' }}>
                              {new Date(groupMessage.GroupReplyCreatedAt).toLocaleDateString()}{' '}
                              {new Date(groupMessage.GroupReplyCreatedAt).toLocaleTimeString(
                                undefined,
                  ...
                  <Card
                    ...
                    style={{
                      ...
                      marginBottom: '10px',
                      ...(groupMessage.GroupReplyID != null
                  ...
                  >
                    <Group>
                      ...
                      {groupMessage.GroupMessageFirstname +
                        ' ' +
                        groupMessage.GroupMessageLastname ==
                      groupData[0].UserFullname ? (
                        <Box
                          style={{
                            width: '100%',
                            wordWrap: 'break-word',
                        ...
                        >
                          <Text>
                            ...
                            <span style={{ fontWeight: 400, fontSize: 13, float: 'right' }}>
                              {new Date(groupMessage.GroupMessageCreatedAt).toLocaleDateString()}{' '}
                              {new Date(groupMessage.GroupMessageCreatedAt).toLocaleTimeString(
                                undefined,
                                { hour: '2-digit', minute: '2-digit', hour12: true }
                              )}
                            </span>
                          ...
                          {editingMessageId == groupMessage.GroupMessageID ? (
                            ...
                            <Text style={{ flex: 1 }}>{groupMessage.GroupMessageContent}</Text>
                          )}
                          ...
                          {editingMessageId == groupMessage.GroupMessageID ? (
                            ...
                            <>
                                ...
                                onClick={() =>
                                  handleReply(
                                    ...
                                    groupMessage.GroupMessageFirstname +
                                      ...
                                      groupMessage.GroupMessageLastname,
                                    groupMessage.GroupMessageContent,
                                    groupMessage.GroupMessageCreatedAt
                                  )
                                }
                                variant="subtle"
                                color="grape"
                              ...
                              <Button
                                onClick={() =>
                                  handleDeleteGroupMessage(groupMessage.GroupMessageID)
                                }
                                variant="subtle"
                                color="red"
                                size="sm"
